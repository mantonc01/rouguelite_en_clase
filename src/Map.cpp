...
public :
    ...
    bool visitNode(TCODBsp *node, void *userData) {
    	if ( node->isLeaf() ) {
			...
			if ( roomNum != 0 ) {//si no es la primera habitación haz un pasillo
...
void Map::createRoom(bool first, int x1, int y1, int x2, int y2){
  dig(x1,y1,x2,y2);//taladrar esas posiciones
  if(first){
    ...
    TCODRandom *rng=TCODRandom::getInstance();
    if (rng->getInt(0,2)==0){
      Actor* npc2 = new Actor((x1+x2)/2, (y1+y2)/2, 'E', TCODColor::red);
      engine.actors.push(npc2);
    }
